...
export default function AdminOrders({ setCurrentPage }: AdminOrdersProps) {
  ...
  const loadOrders = async () => {
    try {
      ...
      const response = await fetch('https://store25-0ven.onrender.com/api/admin/orders', {
        headers: {
  ...
  const updateOrderStatus = async (orderId: string, newStatus: string) => {
    try {
      ...
      const response = await fetch(`https://store25-0ven.onrender.com/api/admin/orders/${orderId}/status`, {
        method: 'PUT',
        headers: {
  ...
  const downloadOrderReport = async () => {
    ...
    try {
      ...
      const response = await fetch(`https://store25-0ven.onrender.com/api/orders/report?startDate=${reportStartDate}&endDate=${reportEndDate}`, {
        headers: {
          'Authorization': `Bearer ${token}`,
